#!/usr/bin/env python3
"""
classical 1D-Block prototype (mirrors analyze_rcm.py/analyze_fennel.py
exactly, for comm=block_nccl).

Parses bonus_partitioning_strategies/outputs/block/block-*.out (comm=block_nccl
kernel=acc RESULT lines, from sbatch/block_small_matrices.sh) and
...
def main():
    block_dir = os.path.join(os.path.dirname(__file__), "outputs", "block")
    paths = sys.argv[1:] or sorted(glob.glob(os.path.join(block_dir, "block-*.out")))
    if not paths:
        print(f"no input files (pass paths, or run from the project root with "
              f"{block_dir}/block-*.out present)", file=sys.stderr)
        sys.exit(1)
    ...
    if strong is None:
        print(f"note: {strong_path} not found -- reporting Block-only numbers, "
    ...
    summary_rows = []
    print(f"{'matrix':<18} {'P':>3} {'T_block_ms':>12} {'T_cyclic_ms':>14} "
          f"{'speedup_vs_cyclic':>18} {'ghost_frac_block':>16} {'nnz_max/avg':>12}")
    for (matrix, P), grp in sorted(groups.items()):
        ...
        # Load-balance check (this prototype has NO NNZ balancing at all,
        # unlike RCM/Fennel -- worth watching directly).
        nnz_list = [r["nnz_local"] for r in rep_chunks[0]]
        nnz_ratio = (max(nnz_list) / (sum(nnz_list) / len(nnz_list))) if nnz_list else float("nan")

        t_cyclic = strong.get(("ghost_nccl", "acc", matrix, P)) if strong else None
        speedup = (t_cyclic / t_block) if (t_cyclic and t_block > 0) else None

        print(f"{matrix:<18} {P:>3} {t_block:>12.4f} "
              f"{(t_cyclic if t_cyclic else float('nan')):>14.4f} "
              f"{(speedup if speedup is not None else float('nan')):>18.3f} "
              f"{ghost_frac:>16.4f} {nnz_ratio:>12.3f}")
        summary_rows.append([matrix, P, t_block, t_cyclic, speedup, ghost_frac, nnz_ratio])

    csv_dir = os.path.join(os.path.dirname(__file__), "outputs", "csv")
    out_csv = os.path.join(csv_dir, "block_vs_cyclic_summary.csv")
    os.makedirs(csv_dir, exist_ok=True)
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["matrix", "P", "T_block_median_ms", "T_cyclic_ghost_nccl_acc_median_ms",
                     "speedup_block_over_cyclic", "ghost_fraction_block", "nnz_max_over_avg"])
