/**
 * 1D cyclic partitioning + selective ghost-entry exchange (MPI point-to-
 * point Isend/Irecv, only to/from peers with nonzero counts), cuSPARSE
 * kernel.
 *
 * Usage:
 *   mpirun -n <P> ./bin/spmv_ghost_cusparse <matrix.mtx>
 */
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char **argv)
{
    ...
    {
        if (rr == rank)
            print_result("ghost_mpi", "cusparse", path, size, rank,
