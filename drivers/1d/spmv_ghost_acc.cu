/**
 * Identical ghost-exchange communication code to spmv_ghost_cusparse.cu,
 * ACC kernel (LINE/FLAT, chosen per-matrix at setup time) instead of
 * cuSPARSE.
 *
 * Usage:
 *   mpirun -n <P> ./bin/spmv_ghost_acc <matrix.mtx>
 */
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char **argv)
{
    ...
    auto do_exchange = [&]()
    {
        ...
        for (int p = 0; p < size; p++)
        {
            ...
            {
                MPI_Request req;
                MPI_Irecv(d_x_full + local.nrows_local + plan.need_displs[p],
                          plan.need_counts[p], MPI_FLOAT, p, 0, MPI_COMM_WORLD, &req);
        ...
        for (int p = 0; p < size; p++)
        {
            ...
            {
                MPI_Request req;
                MPI_Isend(d_sendbuf + plan.req_displs[p],
                          plan.req_counts[p], MPI_FLOAT, p, 0, MPI_COMM_WORLD, &req);
                requests.push_back(req);
            }
        }
    ...
    {
        if (rr == rank)
            print_result("ghost_mpi", "acc", path, size, rank,
