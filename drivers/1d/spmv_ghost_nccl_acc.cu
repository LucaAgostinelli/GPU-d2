/**
 * NCCL ghost exchange (identical to spmv_ghost_nccl_cusparse.cu) with the
 * ACC kernel (identical to spmv_ghost_acc.cu) instead of cuSPARSE.
 *
 * Usage:
 *   mpirun -n <P> ./bin/spmv_ghost_nccl_acc <matrix.mtx>
 */
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char **argv)
{
    ...
    {
        if (rr == rank)
            print_result("ghost_nccl", "acc", path, size, rank,
