/**
 * Same ghost-exchange planning as spmv_ghost_cusparse.cu, but the runtime
 * value exchange goes through NCCL point-to-point (grouped ncclSend/
 * ncclRecv) instead of MPI Isend/Irecv.
 *
 * Usage:
 *   mpirun -n <P> ./bin/spmv_ghost_nccl_cusparse <matrix.mtx>
 */
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char **argv)
{
    ...
    // GPU binding must happen before ncclCommInitRank.
    ...
    GhostPlan plan = build_ghost_plan(local, MPI_COMM_WORLD);
    ...
    if (rank == 0)
    {
        ...
        {
            x_sendcounts[r] = local_nrows(local.nrows_global, size, r);
            x_displs[r] = off;
            off += x_sendcounts[r];
        }
        x_flat.resize(off);
    ...
    // Grouped ncclSend/ncclRecv, only to/from peers with nonzero counts --
    // same sparse pattern as the MPI point-to-point variant, only the
    // transport differs.
    auto do_exchange = [&]()
    {
        if (n_send > 0)
        ...
        CUDA_CHECK(cudaStreamSynchronize(0)); // NCCL ops are async on the stream
    };
    ...
    {
        if (rr == rank)
            print_result("ghost_nccl", "cusparse", path, size, rank,
