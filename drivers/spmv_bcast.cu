/**
 * Reference driver: distributed SpMV with a full MPI_Bcast of x every
 * call, no selective exchange at all. cuSPARSE kernel is used.
 *
 * Each rank:
 *   1. Binds to its own GPU.
 *   2. Gets its 1D-cyclic local CSR shard via distribute_matrix().
 *   3. Runs a joint warmup+benchmark loop (src/include/joint_benchmark.hpp): each
 *      rep does one MPI_Bcast of the entire x vector + host->device
 *      refresh (comm), immediately followed by one single-shot cuSPARSE
 *      SpMV call (compute), timed as one pipeline.
 *   4. Reports one RESULT line with compute/comm/total stats.
 *
 * Rank 0 additionally gathers all y_local shards back, reconstructs the
 * global result, and checks it against the CPU reference.
 *
 * Usage:
 *   mpirun -n <P> ./bin/spmv_bcast <matrix.mtx>
 */
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char **argv)
{
    ...
    CusparseSpMVContext ctx = spmv_cusparse_setup(d_csr, d_x, d_y);
    ...
    {
        if (rr == rank)
            print_result("bcast", "cusparse", path, size, rank,
