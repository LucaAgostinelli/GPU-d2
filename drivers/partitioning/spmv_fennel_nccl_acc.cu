/**
 * Identical NCCL ghost-exchange pipeline and ACC kernel to spmv_ghost_nccl_acc.cu / spmv_rcm_nccl_acc.cu -- the ONLY
 * variable changed is the row/column ownership rule, replacing the baseline
 * owner(i) = i mod P cyclic partitioning with a streaming GREEDY edge-cut
 * partitioning (LDG, see fennel_partition.hpp). Same one-variable-
 * at-a-time isolation methodology as spmv_rcm_nccl_acc.cu.
 *
 * Usage:
 *   mpirun -n <P> ./bin/spmv_fennel_nccl_acc <matrix.mtx>
 */
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char **argv)
{
    ...
    // Greedy LDG partition (root-only computation, owner_of/local_of/
    // block_start broadcast to every rank) -- computed from `coo` BEFORE
    // distribute_matrix_partitioned() consumes/clears it.
    FennelPartition rp = compute_fennel_partition(coo, nrows, 0, MPI_COMM_WORLD);
    ...
    AccSpMVContext ctx = spmv_acc_setup(d_csr, h, d_x_full, d_y);
    ...
    {
        if (rr == rank)
            print_result("fennel_nccl", "acc", path, size, rank,
