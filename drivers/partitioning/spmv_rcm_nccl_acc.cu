/**
 * Identical NCCL ghost-exchange pipeline and ACC kernel to spmv_ghost_nccl_acc.cu -- the ONLY variable changed is the
 * row/column ownership rule, replacing the baseline owner(i) = i mod P
 * cyclic partitioning with a Reverse Cuthill-McKee (RCM) reordering +
 * NNZ-balanced contiguous-block partitioning (see rcm_partition.hpp).
 *
 * Usage:
 *   mpirun -n <P> ./bin/spmv_rcm_nccl_acc <matrix.mtx>
 */
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char **argv)
{
    ...
    // RCM+NNZ-balanced partition (root-only computation, owner_of/local_of/
    ...
    {
        if (rr == rank)
            print_result("rcm_nccl", "acc", path, size, rank,
