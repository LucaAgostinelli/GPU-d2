/**
 * Identical NCCL ghost-exchange pipeline and ACC kernel to spmv_rcm_nccl_acc.cu / spmv_fennel_nccl_acc.cu -- the ONLY
 * variable changed is the row/column ownership rule, this time 
 * "1D-Block" partitioning: contiguous row ranges over the matrix's ORIGINAL order, 
 * split as evenly as possible BY ROW COUNT (no NNZ balancing, no reordering -- see block_partition.hpp).
 *
 * Usage:
 *   mpirun -n <P> ./bin/spmv_block_nccl_acc <matrix.mtx>
 */
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char **argv)
{
    ...
    // Classical 1D-Block partition -- pure arithmetic, every rank computes
    // it independently (see block_partition.hpp), no root-only step needed
    // beyond broadcasting 'n' itself.
    BlockPartition rp = compute_block_partition(coo, nrows, 0, MPI_COMM_WORLD);
    ...
    {
        if (rr == rank)
            print_result("block_nccl", "acc", path, size, rank,
