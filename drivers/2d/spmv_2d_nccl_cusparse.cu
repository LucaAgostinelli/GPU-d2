/**
 * NCCL variant of spmv_2d_cusparse.cu: ncclBroadcast (col axis) /
 * ncclReduce (row axis) instead of MPI_Bcast/MPI_Reduce. Two independent
 * NCCL communicators are needed (unlike 1D's single world-sized one),
 * since the collectives here are scoped to subgroups, not all P ranks --
 * each bootstrapped via src/include/nccl_bootstrap.hpp over the matching MPI
 * sub-communicator (col_comm or row_comm), not MPI_COMM_WORLD.
 *
 * Usage:
 *   mpirun -n <P> ./bin/spmv_2d_nccl_cusparse <matrix.mtx>
 */
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char **argv)
{
    ...
    float peak_bw_gbs = bind_gpu_and_get_peak_bw(rank); // must precede ncclCommInitRank
    ...
    {
        if (rr == rank)
            print_result("checkerboard_nccl", "cusparse", path, size, rank,
