/**
 * 2D checkerboard SpMV: P_r x P_c process grid (proc_grid.hpp),
 * distribute_matrix_2d() for partitioning, cuSPARSE for the local kernel.
 *
 * Per-call pipeline (all steps timed jointly):
 *   1. MPI_Bcast the owned x segment within col_comm (P_r peers sharing
 *      this rank's pc) -- delivers the full local column range in one
 *      call, no ghost bookkeeping: unlike 1D, every column this rank's
 *      block references belongs to the same fixed peer set.
 *   2. Local block SpMV (cuSPARSE) -> partial y for this rank's row range
 *      (partial because that row range's nonzeros are split across P_c
 *      ranks, unlike 1D where a row belongs entirely to one rank).
 *   3. MPI_Reduce (sum) the partial y within row_comm (P_c peers sharing
 *      this rank's pr) -> final y.
 *
 * Usage:
 *   mpirun -n <P> ./bin/spmv_2d_cusparse <matrix.mtx>
 */
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char **argv)
{
    ...
    // One-time initial scatter of x: global rank 0 sends each column
    // group's segment to that group's pr==0 representative (global rank ==
    // pc, since pr=0 -> rank = 0*P_c+pc = pc). Every other rank gets the
    // real data from the col_comm Bcast inside the timed loop below (first
    // warmup rep).
    ...
    auto do_bcast = [&]()
    {
        MPI_Bcast(d_x_full, local.ncols_local, MPI_FLOAT, 0, grid.col_comm);
    };

    auto do_reduce = [&]()
    {
        MPI_Reduce(d_y_partial, d_y_final, local.nrows_local, MPI_FLOAT, MPI_SUM, 0, grid.row_comm);
    };
    ...
    CusparseSpMVContext ctx = spmv_cusparse_setup(d_csr, d_x_full, d_y_partial);
    ...
    {
        if (rr == rank)
            print_result("checkerboard_mpi", "cusparse", path, size, rank,
    ...
    // Gather final y back to rank 0. Only pc==0 ranks hold a valid reduced
    // block (row_comm's root); every other rank contributes 0 elements.
    ...
    if (rank == 0)
    {
        // Un-interleave 2D-cyclic row order: the r-th contributing rank
        // (pc==0, pr==r/P_c) owns rows {pr, pr+P_r, ...}.
